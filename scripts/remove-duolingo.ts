...
async function removeDuolingo() {
  console.log('Starting removal of Duolingo AI...\n');

  // Find Duolingo AI tool
  const { data: tool, error: fetchError } = await supabase
    .from('tools')
    .select('id, name, slug')
    .or('slug.eq.duolingo-ai,name.ilike.%duolingo%')
    .limit(1)
    .maybeSingle();
  ...
  if (!tool) {
    console.log('✅ Duolingo AI not found in database.');
    return;
  }
  ...
  if (deleteError) {
    ...
    console.log(`\n✅ Done! Deleted Duolingo AI from database.`);
  }
}

removeDuolingo().catch(console.error);
